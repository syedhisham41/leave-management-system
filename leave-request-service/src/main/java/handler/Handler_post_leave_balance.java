...
public class Handler_post_leave_balance implements HttpHandler {

	private final Service_leave_balance leave_balance_service;
	private final ExceptionMapperManager exceptionManager;

	public Handler_post_leave_balance(Service_leave_balance leave_balance_service, ExceptionMapperManager manager) {
	...
	public void handle(HttpExchange exchange) throws IOException {
		...
		try {
			...
			employee_id = om.readValue(is, DTO_employee_created_event.class);
			int result = this.leave_balance_service.createLeaveBalance(employee_id.getEmployee_id(), skipValidation);

			if (result == 6) {
				Utils_handler.APIResponse(exchange, Success_code.LEAVE_BALANCE_CREATED, result);
				return;
			} else {
